...
if [ -e FINAL_RESULTS_MMPBSA.dat ]; then
   temp1=(` grep "DELTA TOTAL" FINAL_RESULTS_MMPBSA.dat | head -n 1 | awk '{print $3}'`)
   temp2=(` grep "DELTA TOTAL" FINAL_RESULTS_MMPBSA.dat | head -n 1 | awk '{print $4}'`)
   temp3=(` grep "DELTA TOTAL" FINAL_RESULTS_MMPBSA.dat | head -n 1 | awk '{print $5}'`)
echo "Average binding energy (MM-GBSA): $temp1 +/- $temp2 kcal/mol (standard error of mean $temp3 kcal/mol)" 
fi 
